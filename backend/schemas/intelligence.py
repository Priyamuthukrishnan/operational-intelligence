...
class TicketRiskResponse(BaseModel):
    ...
    risk_score: float = Field(..., description="Escalation risk score converted to a 0-10 display scale")
    risk_band: str = Field(..., description="Risk classification band (LOW, MEDIUM, HIGH, CRITICAL)")
    confidence_score: float = Field(..., description="Confidence decay score scaled to 0-10 display range")
